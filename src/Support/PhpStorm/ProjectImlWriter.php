...
class ProjectImlWriter extends ConfigWriter
{
	public function write(): bool
	{
		...
		$this->module_registry->modules()
			->sortBy('name')
			->each(function(ModuleConfig $module_config) use (&$content, $modules_directory, $existing_urls, $iml) {
                if ($module_config->name === 'hubhus-modular') {
                    return true;
                }
                ...
                $testsNamespace = rtrim($module_config->namespaces->first(), '\\').'\\Tests';
				if (! $existing_urls->contains($tests_url)) {
					...
					$tests_node->addAttribute('packagePrefix', $testsNamespace);
				} else {
                    /** @var SimpleXMLElement $existing */
                    $existing = $iml->xpath("//component[@name=\"NewModuleRootManager\"]//content[@url=\"file://\$MODULE_DIR\$\"]//sourceFolder[@url=\"$tests_url\"]")[0];
                    if (!$existing->attributes()->packagePrefix) {
                        $existing->addAttribute('packagePrefix', $testsNamespace);
                    }
                }
			});
